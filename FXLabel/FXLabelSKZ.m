    ...
    while (true)
    {
        ...
        for (i = 0; i < charCount; i++)
        {
            ...
            if (i == charCount - 1)
            {
                ...
                x += charWidth;
                break;
            } else if (x + charWidth > width && minFontSize < subFont.pointSize) {
                CGFloat pointSize = MAX(minFontSize, subFont.pointSize - 1.0f);
                subFont = [font fontWithSize:pointSize];
                return [self FXLabel_sizeWithFont:subFont minFontSize:minFontSize
                        ...
                        color:(UIColor *)color
{
    ...
    if (fontSize < font.pointSize && (usingMask || drawCharByChar || FXLABEL_MIN_TARGET_IOS7))
    {
        switch (baselineAdjustment)
        {
            ...
            {
                y += (font.lineHeight - subFont.lineHeight) / 4.0f;
                break;
            }
